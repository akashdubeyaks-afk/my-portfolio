...
const data = {
  "epe-foam": { name: "EPE Foam", type: "Sheet & Fitments", desc: "Polyethylene foamed cotton, new eco packaging with non-cross linking closed-cell structure.", long: "Consists of numerous independent bubbles produced after physical foaming of low density polyethylene. Advantages: moisture protection, shockproof, sound insulation, thermal insulation, recycling usage, environmentally friendly, high resistance to collision, chemical resistant. Ideal substitute of traditional packaging materials. Can replace polystyrene foam and rigid/flexible polyurethane.", specs: ["Density: 18-35 kg/m³", "Thickness: 0.5-100mm", "Color: White, Pink, Black", "Size: Custom as per order"], uses: ["Printers", "Computers", "Engineering", "Furniture", "Medical", "Artillery Box"] },
  "ep-foam": { name: "EP Foam", type: "Custom Moulded", desc: "Expanded polystyrene custom moulded as per product shape for 100% safety.", long: "We take product dimensions, create CAD cavity, ensure zero movement inside box. Drop test 1.2m certified. Low maintenance, used as fitments for fragile items.", specs: ["Density: 15-35 kg/m³", "Moulded Tolerance ±0.5mm", "Custom Cavity", "Any Shape"], uses: ["Electronics", "Glassware", "Machinery"] },
    ...
    <div className="bg-[#FAF9F6] pt-[88px] md:pt-[112px]">
      <div className="px-6 md:px-10 max-w-[1600px] mx-auto">
        <div className="border-t border-black/10 pt-4 flex justify-between mono text-[10px] uppercase opacity-40">
          <Link to="/products" className="hover:opacity-100">← Back to Products</Link><span>Product — {p.name}</span>
        </div>

        <div className="mt-12 grid lg:grid-cols-[1.1fr_0.9fr] gap-12">
          <div>
            <div className="mono text-[10px] uppercase tracking-widest px-3 py-1 rounded-full bg-black text-white inline-block">{p.type}</div>
          ...
          <div className="space-y-6">
            <div className="aspect-[4/3] rounded-[16px] bg-[#EDE9E3] border border-black/5 overflow-hidden">
              <img src={`https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1000`} alt={p.name} className="w-full h-full object-cover" />
            </div>
            <div className="grid grid-cols-3 gap-[1px] bg-black/10 border border-black/10 rounded-[12px] overflow-hidden">
              <div className="bg-[#FAF9F6] p-4 text-center"><div className="syne font-bold">100%</div><div className="mono text-[9px] uppercase opacity-50">Custom</div></div>
              <div className="bg-[#FAF9F6] p-4 text-center"><div className="syne font-bold">48HR</div><div className="mono text-[9px] uppercase opacity-50">Sample</div></div>
              <div className="bg-[#FAF9F6] p-4 text-center"><div className="syne font-bold">Zero</div><div className="mono text-[9px] uppercase opacity-50">Damage</div></div>
            </div>
          </div>
        </div>

        <div className="border-t border-black/10 mt-16 py-8 flex justify-between mono text-[10px] uppercase opacity-40">
          <Link to="/products">← All Products</Link><span>Need custom drawing? Send product photo + dimensions</span>
        </div>
      </div>
    </div>
  );
